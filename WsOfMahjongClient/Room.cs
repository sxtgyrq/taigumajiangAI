...
#warning 这里要优化！！！
        ...
        internal static void PositionSelectF(State s, PositionSelect ps)
        {
            CommonClass.PositionSelectToServer psts = new PositionSelectToServer()
            {
                c = "PositionSelectToServer",
                Key = s.Key,
                Position = ps.Position,
                GroupKey = s.GroupKey
            };
            var msg = Newtonsoft.Json.JsonConvert.SerializeObject(psts);
            Startup.sendInmationToUrlAndGetRes(Room.roomUrls[s.roomIndex], msg);
            //throw new NotImplementedException();
        }

        internal static void StartPositionSelectF(State s, StartPositionSelect sps)
        {
            //StartPositionSelectToServer
            CommonClass.StartPositionSelectToServer psts = new StartPositionSelectToServer()
            {
                c = "StartPositionSelectToServer",
                Key = s.Key,
                GroupKey = s.GroupKey
            };
            var msg = Newtonsoft.Json.JsonConvert.SerializeObject(psts);
